...
++  artifact-content
  |=  raw=@t
  ^-  @t
  (crip :(weld (trip '{"noltbookApp":1,"app":{"desk":"calendar","publisher":null},"data":{"kind":"calendar-event","event":') (trip raw) "}}"))
++  edit-artifact-card
  |=  [our=@p art-id=@ta raw=@t]
  ^-  card
  [%pass /noltbook-edit/[art-id] %agent [our %noltbook] %poke %noltbook-api !>([%edit-artifact ~ `[%calendar `'Calendar' ~] art-id (artifact-content raw)])]
...
++  follow-card
  ...
  ?:  =(who our)  ~
  ~[[%pass /calendar-follow/(scot %p who) %agent [who %calendar] %poke %noun !>(`cal-poke`[%follow event-key raw kind])]]
++  unfollow-card
  ...
  =/  who=@p  (event-creator our raw)
  ?:  =(who our)  ~
  ~[[%pass /calendar-unfollow/(scot %p who) %agent [who %calendar] %poke %noun !>(`cal-poke`[%unfollow event-key])]]
++  fanout-cards
  |=  [event-key=@t raw=@t folmap=(map @t (set @p))]
  ^-  (list card)
  =/  whos=(set @p)  (fall (~(get by folmap) event-key) *(set @p))
  (turn ~(tap in whos) |=(who=@p [%pass /calendar-update/(scot %p who) %agent [who %calendar] %poke %noun !>(`cal-poke`[%event-update event-key raw])]))
++  startup-sync-cards
...
++  on-poke
  ...
  ?+  mark  (on-poke:def mark vase)
    ...
    ?-    -.msg
      ...
      ?~  ev  ~
      ~[[%pass /calendar-sync/(scot %p src.bowl) %agent [src.bowl %calendar] %poke %noun !>(`cal-poke`[%event-update event-key.msg u.ev])]]
        %unfollow
      =/  old=(set @p)  (fall (~(get by followers.state) event-key.msg) *(set @p))
      ...
      ~
        %event-update
      =/  was-saved=?  ?=(^ (~(get by events.state) event-key.msg))
      ...
      :_  ?:(was-saved this(events (~(put by events.state) event-key.msg raw.msg)) ?:(was-ref this(ref-events (~(put by ref-events.state) event-key.msg raw.msg)) this))
      cards
        %sync-request
      =/  ev=(unit @t)  (~(get by events.state) event-key.msg)
      :_  this
    ...
    ?:  &(=(%'POST' method.request.inbound-request) =(path-tape "/apps/calendar/api/artifact-ref"))
      ...
      (weld rsp-cards (follow-card our.bowl key raw %ref))
    ?:  &(=(%'POST' method.request.inbound-request) =(path-tape "/apps/calendar/api/test-notification"))
      =/  bod  body.request.inbound-request
      ?~  bod
        =/  =simple-payload:http
          :-  [400 ~[['content-type' 'application/json']]]
