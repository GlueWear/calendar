...
++  on-poke
  ...
  ?+  mark  (on-poke:def mark vase)
    ...
    ?-    -.msg
      ...
      ?~  ev  ~
      (weld ~[[%pass /calendar-sync/(scot %p src.bowl) %agent [src.bowl %calendar] %poke %calendar-poke !>(`cal-poke`[%event-update event-key.msg (event-view event-key.msg u.ev event-notes.state)])]] note-cards)
        %unfollow
      =/  old=(set @p)  (fall (~(get by followers.state) event-key.msg) *(set @p))
