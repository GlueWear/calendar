...
++  json-add-str-field
  ...
  =/  blob=tape  (trip raw)
  ?:  =(0 (lent blob))  raw
  =/  base=tape  (scag (dec (lent blob)) blob)
  (crip :(weld base (trip ',"') (trip key) (trip '":"') (trip val) (trip '"}')))
++  event-view
  |=  [key=@t raw=@t event-notes=(map @t @ta)]
  ^-  @t
  =/  nid=(unit @ta)  (~(get by event-notes) key)
  ?~  nid  raw
  (json-add-str-field raw 'eventNoteId' (crip (trip u.nid)))
++  event-key
  |=  [our=@p raw=@t]
  ^-  @t
  =/  blob=tape  (trip raw)
...
++  json-list
  ...
  (crip :(weld "[" body "]"))
++  json-event-list
  |=  [items=(list [@t @t]) event-notes=(map @t @ta)]
  ^-  @t
  =/  body=tape
    |-  ^-  tape
    ?~  items  ~
    =/  item=@t  (event-view -.i.items +.i.items event-notes)
    ?~  t.items  (trip item)
    :(weld (trip item) "," $(items t.items))
  (crip :(weld "[" body "]"))
++  json-str-list
  |=  items=(list @t)
  ^-  @t
...
++  noltbook-results-watch
  |=  our=@p
  ^-  card
  [%pass /noltbook-api-results %agent [our %noltbook] %watch /api/results]
++  create-event-note-card
  |=  [our=@p rid=@ud title=@t]
  ^-  card
  [%pass /noltbook-create-event-note/(scot %ud rid) %agent [our %noltbook] %poke %noltbook-api !>([%create-note `rid title ~])]
++  add-event-note-member-card
  |=  [our=@p nid=@ta who=@p]
  ^-  card
  [%pass /noltbook-add-event-member/[nid]/(scot %p who) %agent [our %noltbook] %poke %noltbook-api !>([%add-member ~ nid (scot %p who)])]
++  add-event-note-member-cards
  |=  [our=@p nid=@ta whos=(set @p)]
  ^-  (list card)
  (turn ~(tap in whos) |=(who=@p (add-event-note-member-card our nid who)))
++  startup-sync-cards
  |=  [our=@p evmap=(map @t @t) refmap=(map @t @t)]
  ^-  (list card)
  =/  saved=(list [@t @t])  ~(tap by evmap)
...
++  on-poke
  ...
  ?+  mark  (on-poke:def mark vase)
    ...
    ?-    -.msg
      ...
      =/  old-saved=(set @p)  (fall (~(get by saved-followers.state) event-key.msg) *(set @p))
      =/  next-saved=(set @p)
        ?:  =(%save kind.msg)
          (~(put in old-saved) src.bowl)
        old-saved
      =/  ev=(unit @t)  (~(get by events.state) event-key.msg)
      =/  note=(unit @ta)  (~(get by event-notes.state) event-key.msg)
      =/  note-cards=(list card)
        ?:  =(%save kind.msg)
          ?~  note  ~
          ~[(add-event-note-member-card our.bowl u.note src.bowl)]
        ~
      :_  this(followers (~(put by followers.state) event-key.msg next), saved-followers (~(put by saved-followers.state) event-key.msg next-saved))
      ?~  ev  ~
      (weld ~[[%pass /calendar-sync/(scot %p src.bowl) %agent [src.bowl %calendar] %poke %calendar-poke !>(`cal-poke`[%sync-update event-key.msg (event-view event-key.msg u.ev event-notes.state)])]] note-cards)
        %unfollow
      =/  old=(set @p)  (fall (~(get by followers.state) event-key.msg) *(set @p))
      =/  next=(set @p)  (~(del in old) src.bowl)
      =/  old-saved=(set @p)  (fall (~(get by saved-followers.state) event-key.msg) *(set @p))
      =/  next-saved=(set @p)  (~(del in old-saved) src.bowl)
      :_  this(followers (~(put by followers.state) event-key.msg next), saved-followers (~(put by saved-followers.state) event-key.msg next-saved))
      ~
        %event-update
      ...
      cards
        %sync-update
      ~&  [%calendar-sync-update-rcvd from=src.bowl key=event-key.msg]
      =/  old-raw=(unit @t)  (~(get by events.state) event-key.msg)
      =/  was-saved=?  ?=(^ old-raw)
      =/  was-ref=?  (~(has in ref-following.state) event-key.msg)
      =/  blob=tape  (trip raw.msg)
      ...
      :_  this
      ?~  ev  ~
      ~[[%pass /calendar-sync/(scot %p src.bowl) %agent [src.bowl %calendar] %poke %calendar-poke !>(`cal-poke`[%sync-update event-key.msg u.ev])]]
    ==
      %handle-http-request
    ...
    ?:  &(=(%'GET' method.request.inbound-request) =(path-tape "/apps/calendar/api/events"))
      =/  evs=@t  (json-event-list ~(tap by events.state) event-notes.state)
    ...
    ?:  &(=(%'POST' method.request.inbound-request) =(path-tape "/apps/calendar/api/resolve"))
      ...
      =/  found=(unit @t)
        ...
        `u.ev
      ?^  found
        =/  body=@t  (crip :(weld (trip '{"ok":true,"key":"') (trip key) (trip '","event":') (trip (event-view key u.found event-notes.state)) "}"))
        =/  =simple-payload:http
          :-  [200 ~[['content-type' 'application/json']]]
          `(as-octs:mimes:html body)
        [(give-simple-payload:app:server eyre-id simple-payload) this]
    ...
    ?:  &(=(%'POST' method.request.inbound-request) =(path-tape "/apps/calendar/api/event"))
      ...
      =/  cancelled=?  (json-true-field 'cancelled' blob)
      =/  raw-out=@t  (event-view key raw event-notes.state)
      =/  extra=(list card)
        ?:  =(creator our.bowl)
          (weld (fanout-cards key raw-out followers.state) (local-artifact-cards our.bowl key raw-out refs.state))
    ...
    ?:  &(=(%'POST' method.request.inbound-request) =(path-tape "/apps/calendar/api/create-event-note"))
      ...
      ?.  =(creator our.bowl)
        =/  =simple-payload:http
          ...
          `(as-octs:mimes:html '{"ok":false,"error":"only creator can create event note"}')
        [(give-simple-payload:app:server eyre-id simple-payload) this]
      =/  existing=(unit @ta)  (~(get by event-notes.state) key)
      ?^  existing
        =/  res=@t  (crip :(weld (trip '{"ok":true,"noteId":"') (trip u.existing) (trip '"}')))
        =/  =simple-payload:http
          :-  [200 ~[['content-type' 'application/json']]]
          `(as-octs:mimes:html res)
        [(give-simple-payload:app:server eyre-id simple-payload) this]
      =/  title=@t  (fall (json-str-field 'title' (trip raw)) 'Calendar Event')
      =/  rid=@ud  (add (now-ms now.bowl) (lent ~(tap by note-requests.state)))
      =/  nid=@ta  (crip (weld "note-" (trip (scot %da now.bowl))))
      =/  saved=(set @p)  (fall (~(get by saved-followers.state) key) *(set @p))
      =/  res=@t  (crip :(weld (trip '{"ok":true,"noteId":"') (trip nid) (trip '","requestId":') (trip (scot %ud rid)) (trip '}')))
      =/  =simple-payload:http
        :-  [200 ~[['content-type' 'application/json']]]
        `(as-octs:mimes:html res)
      =/  rsp-cards  (give-simple-payload:app:server eyre-id simple-payload)
      :_  this(event-notes (~(put by event-notes.state) key nid), events (~(put by events.state) key raw))
      (weld rsp-cards (weld ~[(create-event-note-card our.bowl rid title)] (add-event-note-member-cards our.bowl nid saved)))
